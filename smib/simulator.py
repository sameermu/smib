...
def run_smib_genrou_avr_pss_gov(genrou: "GENROU", avr, pss, gov,
                                network: Network,
                                t_end: float = 5.0, h: float = 5e-3,
                                scenarios=(),
                                init_V: complex | None = None,
                                init_S: complex | None = None,
                                require_steady_state: bool = True) -> SimResult:
    """Run a SMIB scenario with the full classical plant stack:
    GENROU + ST1A + PSS1A + TGOV1.

    Signal flow each timestep:

        |V_terminal|        →  AVR.inputs.V_terminal_mag
        ω̄ (rotor slip)      →  PSS.inputs.Delta_omega
        ω̄ (rotor slip)      →  GOV.inputs.Delta_omega
        PSS.Vpss            →  AVR.inputs.Vpss
        AVR.Efd             →  GENROU.params.Efd
        GOV.Pm              →  GENROU.params.Pm

    The state vector concatenates GENROU's 4 states, the AVR's 2,
    the PSS's 3, and the governor's 2:

        x = [delta, omega, Eqp, Edp,   Vc, x_LL,   x_w, x_LL1, x_LL2,   x_v, x_r]
              GENROU                 | ST1A      | PSS1A               | TGOV1

    Like the PSS, the governor injects no current — it closes a purely
    mechanical loop (ω̄ → valve → reheat → Pm → swing equation), so the
    algebraic Y_bus solve is unchanged from Phase 2.1/2.2.  What IS new
    is that Pm, a constant in every earlier phase, is now a live model
    output recomputed each corrector iteration.
    """
    from .models.st1a import ST1A
    from .models.pss1a import PSS1A
    from .models.tgov1 import TGOV1
    from .models.genrou import GENROU
    if not isinstance(genrou, GENROU):
        raise TypeError("genrou must be a GENROU instance")
    if not isinstance(avr, ST1A):
    ...
    if not isinstance(pss, PSS1A):
        raise TypeError("pss must be a PSS1A instance")
    if not isinstance(gov, TGOV1):
        raise TypeError("gov must be a TGOV1 instance")

    # Init order: GENROU first (derives Efd_init AND Pm_init), then AVR
    # (back-solves Vref from Efd_init), then PSS (all zeros), then the
    # governor (back-solves Pref from Pm_init).
    if init_V is not None and init_S is not None:
        genrou.initialise(init_V, init_S)
        avr.initialise(init_V, init_S, Efd_init=genrou.params["Efd"])
        pss.initialise()
        gov.initialise(Pm_init=genrou.params["Pm"])

    n_g = len(genrou.state_keys)
    n_a = len(avr.state_keys)
    n_p = len(pss.state_keys)
    n_t = len(gov.state_keys)

    def get_x():
        return np.concatenate([genrou.flatten(), avr.flatten(),
                               pss.flatten(), gov.flatten()])

    def set_x(x):
        genrou.unflatten(x[:n_g])
        avr.unflatten(x[n_g:n_g + n_a])
        pss.unflatten(x[n_g + n_a:n_g + n_a + n_p])
        gov.unflatten(x[n_g + n_a + n_p:])

    def _wire_signals(V):
        """One pass of the algebraic signal flow for the current states."""
        slip = genrou.state["omega"]
        pss.inputs["Delta_omega"] = slip
        gov.inputs["Delta_omega"] = slip
        pss.derivatives()                 # refresh Vpss cache
        gov.derivatives()                 # refresh Pm cache
        avr.inputs["V_terminal_mag"] = abs(V)
        avr.inputs["Vpss"] = pss.algebraic_output()["Vpss"]
        avr.derivatives()                 # refresh Efd cache
        genrou.params["Efd"] = avr.algebraic_output()["Efd"]
        genrou.params["Pm"] = gov.algebraic_output()["Pm"]
        genrou.inputs["V_terminal"] = V
    ...
    def f_fn(x, V):
        ...
        return np.concatenate([
            ...
            np.array([d_a[k] for k in avr.state_keys]),
            np.array([d_p[k] for k in pss.state_keys]),
            np.array([d_t[k] for k in gov.state_keys]),
        ])
    ...
    d0_a = avr.derivatives()
    d0_p = pss.derivatives()
    d0_t = gov.derivatives()
    worst = max(max(abs(v) for v in d0_g.values()),
                max(abs(v) for v in d0_a.values()),
                max(abs(v) for v in d0_p.values()),
                max(abs(v) for v in d0_t.values()))
    if require_steady_state and worst > 1e-5:
        raise RuntimeError(
            f"GENROU+ST1A+PSS1A+TGOV1 not at steady state: "
            f"max|dx/dt| = {worst:.3e}"
        )

    n_steps = int(round(t_end / h))
    t = np.linspace(0.0, t_end, n_steps + 1)

    # Trace dict with collision-aware naming.  Precedence (highest →
    # lowest): GOV, PSS, AVR, GENROU.  The notable collision is "Pm":
    # the governor's value is the live turbine output, GENROU's view of
    # it lands under "genrou_Pm" (they agree at every converged step).
    out0_g = genrou.algebraic_output()
    out0_a = avr.algebraic_output()
    out0_p = pss.algebraic_output()
    out0_t = gov.algebraic_output()
    traces = {}
    for k, v in out0_g.items():
        new_k = f"genrou_{k}" if (k in out0_a or k in out0_p or k in out0_t) else k
        traces[new_k] = np.zeros(n_steps + 1); traces[new_k][0] = v
    for k, v in out0_a.items():
        new_k = f"avr_{k}" if (k in out0_p or k in out0_t) else k
        traces[new_k] = np.zeros(n_steps + 1); traces[new_k][0] = v
    for k, v in out0_p.items():
        new_k = f"pss_{k}" if k in out0_t else k
        traces[new_k] = np.zeros(n_steps + 1); traces[new_k][0] = v
    for k, v in out0_t.items():
        traces[k] = np.zeros(n_steps + 1); traces[k][0] = v
    iters_log = np.zeros(n_steps + 1, dtype=int)
    ...
    for k in range(1, n_steps + 1):
        ...
        out_a = avr.algebraic_output()
        out_p = pss.algebraic_output()
        out_t = gov.algebraic_output()
        for kk, vv in out_g.items():
            target = f"genrou_{kk}" if (kk in out_a or kk in out_p or kk in out_t) else kk
            traces[target][k] = vv
        for kk, vv in out_a.items():
            target = f"avr_{kk}" if (kk in out_p or kk in out_t) else kk
            traces[target][k] = vv
        for kk, vv in out_p.items():
            target = f"pss_{kk}" if kk in out_t else kk
            traces[target][k] = vv
        for kk, vv in out_t.items():
            traces[kk][k] = vv

    return SimResult(
        t=t,
        traces=traces,
        final_state={**genrou.state, **avr.state, **pss.state, **gov.state},
        info={"mean_iters": float(iters_log.mean()),
              "max_iters": int(iters_log.max())},
    )
